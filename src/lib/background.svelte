	...
	<svg class="section_sky" preserveAspectRatio="none">
		<defs>
			...
			<linearGradient class="gradient" id="gradient-sun" gradientTransform="rotate(90)">
				<stop offset="5%" stop-color="var(--sunDark)" />
...
<style>
	.tree-background {
		--colorTransitionSpeed: 1s;
		position: fixed;
		z-index: -1;
		transition: fill var(--colorTransitionSpeed) ease-in-out,
			background-color var(--colorTransitionSpeed) ease-in-out,
			filter var(--colorTransitionSpeed) ease-in-out;
		filter: brightness(1);
		height: 100vh;
		display: grid;
	...
	.gradient stop {
		transition: stop-color var(--colorTransitionSpeed) ease-in-out;
	}
	...
	.moon {
		...
		transition: opacity var(--colorTransitionSpeed) ease-in-out;
	}
